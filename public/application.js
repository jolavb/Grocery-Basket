...
var config = {
  apiOrigins: {
    production: 'https://grocery-basket.herokuapp.com/',
...
var getFormFields = __webpack_require__(17);
...
var onSignout = function onSignout(event) {
  ...
  api.removeAllCartItems().then(ui.updateCartSuccess).catch(ui.updateCartFail);
...
// Stores Events
var onGetStores = function onGetStores() {
  api.GetStores().then(ui.GetStoreSuccess).then(function (data) {
    ui.loader(false, '.store-view');
    return data;
  }).then(map.loadMap).catch(ui.GetStoreSuccessFail);
};

// Cart Events
var onCartModal = function onCartModal() {
  ui.showCartModal();
};
...
// Items API Calls
var GetStoreItems = function GetStoreItems(storeId) {
  return $.ajax({
    method: 'GET',
...
var GetStoreRecipes = function GetStoreRecipes(storeId) {
  return $.ajax({
    method: 'GET',
...
var addCartItemQuantity = function addCartItemQuantity(itemId, quantity) {
  var data = {
    cart_item: {
      quantity: quantity
    }
...
var removeAllCartItems = function removeAllCartItems() {
  return $.ajax({
    method: 'DELETE',
...
module.exports = {
  ...
  addCartItemQuantity: addCartItemQuantity,
  GetStoreRecipes: GetStoreRecipes
};
...
var showStoresTemplate = __webpack_require__(18);
var showStoreItemsTemplate = __webpack_require__(35);
var showCartTemplate = __webpack_require__(37);
var showStoreView = __webpack_require__(38);
var showStoreRecipesTemplate = __webpack_require__(39);
...
// Auth Modal Controls
var showModal = function showModal(formClass) {
  // FormClass is ID of event button
  var targetForm = $('.' + formClass);
  var formtitle = {
...
var signInSuccess = function signInSuccess(response) {
  ...
  var showCartHtml = showCartTemplate();
...
var changePassSuccess = function changePassSuccess(response) {
  clearForms();
  // Delete Users Current Cart Items
  api.removeAllCartItems().then(updateCartSuccess).catch(updateCartFail);

  // Signout User
  api.signout().then(signoutSuccess).catch(signoutFail);
};

var changePassFail = function changePassFail(response) {
  clearForms();
...
var GetStoreSuccess = function GetStoreSuccess(data) {
  // Render Stores Template to content to display stores
  var showStoresHtml = showStoresTemplate({ stores: data.stores });
  ...
  $('#content').on('click', '.store', function () {
    ...
    loader(true);
    api.GetStoreItems(store).then(GetItemsSuccess).catch(GetItemsFail);
  });
  return data;
};

var GetStoreSuccessFail = function GetStoreSuccessFail() {
...
var GetItemsSuccess = function GetItemsSuccess(data) {
  ...
  var showStoreItemsHTML = showStoreItemsTemplate({ items: data.items });
  var showStoreViewHTML = showStoreView({ items: data.items });
  ...
  $('.display-stores').on('click', function () {
  ...
  $('.items').on('click', 'button', function () {
    ...
    var removed = changeItemGlyph(this);
    if (removed) {
      $(this).removeClass('glyphicon-minus');
      api.removeCartItem(cartItem).then(updateCartSuccess).catch(RemoveItemFail);
    } else {
      api.addItemToCart(cartItem).then(updateCartSuccess).catch(addItemFail);
    }
  });
...
var switchSaleView = function switchSaleView() {
  if (!$(this).hasClass('active')) {
    ...
    switch ($(this).attr('id')) {
      case 'sale-items':
        api.GetStoreItems(store.StoreId).then(GetItemsSuccess).catch(GetItemsFail);
        break;

      case 'sale-recipes':
        api.GetStoreRecipes(store.StoreId).then(GetRecipesSuccess).catch(GetRecipesFail);
        break;
    }
  }
};
...
var GetRecipesSuccess = function GetRecipesSuccess(data) {
  var showRecipeHTML = showStoreRecipesTemplate({ 'recipes': data.recipes });
...
var GetItemsFail = function GetItemsFail(response) {
...
var CheckItems = function CheckItems(items) {
  $('.cart-btn-remove').each(function (index, element) {
    var cartItemId = element.getAttribute('data-id');
    var item = $('.panel button[data-id=' + cartItemId + ']');
    if (item) {
...
var changeItemGlyph = function changeItemGlyph(item, reset) {
...
var updateCartSuccess = function updateCartSuccess(cartItems) {
  $('.items-count').html(cartItems.cart_items.length);
  var showCartHtml = showCartTemplate({ cartItems: cartItems.cart_items });
  $('#shopping-cart').html(showCartHtml);
  ...
  $('.cart-items').on('click', '.cart-btn-remove', function () {
    var cartItem = $(this).attr('data-id');
    api.removeCartItem(cartItem).then(updateCartSuccess, changeItemGlyph('.panel button[data-id=' + cartItem + ']')).catch(RemoveItemFail);
  });

  // Register Add from cart button events. On success invokes self.
  $('.cart-items').on('click', '.cart-btn-add', function () {
    var cartItem = $(this);
    var quantity = parseInt($(this).siblings('p').text()) + 1;

    api.addCartItemQuantity(cartItem.attr('data-id'), quantity).then(function (response) {
      cartItem.siblings('p').text(response.cart_item.quantity);
    }).catch();
  });
};

var updateCartFail = function updateCartFail(response) {
  errorHandle('Error Updating Cart');
};

var addItemFail = function addItemFail(response) {
  errorHandle('Error Adding to Basket');
};

var RemoveItemFail = function RemoveItemFail(response) {
  errorHandle('Error Removing from Basket');
};

// To Do
var CalculateExpenses = function CalculateExpenses(cartItems) {
...
// Show/Hide Loader
var loader = function loader(show, target) {
  if (!show) {
    $('#loader').hide();
    switchView(target);
  } else {
...
var switchView = function switchView(target) {
  $('.main-view').hide();
  $('#content').show();
  $(target).show();
};
...
module.exports = {
  ...
  loader: loader,
  updateCartFail: updateCartFail,
  GetStoreSuccessFail: GetStoreSuccessFail
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(1)))
...
var loadGoogleMapsAPI = __webpack_require__(40);
var GeoJSON = __webpack_require__(41);

var init = function init(gmap, data) {

  var image = 'https://cdn0.iconfinder.com/data/icons/shopping-vol-1-2/128/P-1-09-128.png';
  ...
  var center = { lat: 42.35, lng: -71.12 };
  ...
  data.features.forEach(function (store) {
    ...
    var latLng = new gmap.LatLng(coords[1], coords[0]);
    var marker = new gmap.Marker({
      position: latLng,
      map: map,
      icon: icon
    });
    var html = '<h4 class= "store-title">' + store.properties.name + '</h4> <p class= "store-loc">' + store.properties.location + '</p>';
    bindInfoWindow(marker, html);
  });
};

var loadMap = function loadMap(data) {
  var geoData = GeoJSON.parse(data.stores, { Point: ['lat', 'lng'] });
  var options = { key: 'AIzaSyAGRlNMq-_hdvBpP2L1ZPPhcJxBhZh4YyY' };

  loadGoogleMapsAPI(options).then(function (map) {
    init(map, geoData);
  }).catch(console.error);
};
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n<div id=\"map\"></div>\n<table class = 'main-view store-view table table-striped table-condensed table-bordered'>\n   <thead>\n     <tr>\n       <td><h3>Store</h3></td>\n       <td><h3>Location</h3></td>\n     </tr>\n   </thead>\n\n   <tbody>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.stores : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "   </tbody>\n </table>\n";
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  <div class='col-md-3 col-sm-6 col-xs-12 itemcol'>\n      <div class=\"item panel panel-default\">\n        <div class=\"panel-heading\">\n          <p class=\"panel-title truncate\">"
    ...
    + "\" class = 'img' height='200px' >\n        </div>\n        </a>\n          <button data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\"\n          class=\"item-btn btn btn-mini btn-success glyphicon glyphicon-plus\"\n          type=\"button\">\n          </button>\n      </div>\n    </div>\n      <!-- "
    + ((stack1 = __default(__webpack_require__(36)).call(depth0 != null ? depth0 : (container.nullContext || {}),(data && data.index),{"name":"custom","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + " -->\n";
  ...
  return "\n<!-- <section class = 'main-view items-view' > -->\n<div class = 'row items'>\n"
...
var custom = function custom(index, options) {
  var newrow = (index + 1) % 4;
  if (newrow === 0) {
    return options.fn(this);
  } else {
    return options.inverse(this);
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "    <tr class = 'cart-item' data-id= "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + ">\n      <td> <button class='cart-btn-remove btn btn-danger' data-id = "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.id : stack1), depth0))
    + ">Remove</button></td>\n      <td>"
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.quantity : stack1), depth0))
    + "</td>\n      <td>"
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.title : stack1), depth0))
    + "</td>\n      <td>$"
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.price : stack1), depth0))
    + "</td>\n      <td class = 'quantity'>\n        <p>"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.quantity : stack1), depth0))
    + "</p>\n        <button data-id = "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.id : stack1), depth0))
    + " class= 'cart-btn-add btn btn-mini btn-success glyphicon glyphicon-plus'></button>\n      </td>\n      <!-- <td> <input type='text' placeholder=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.quantity : stack1), depth0))
    + "\"></input></td> -->\n    </tr>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<table class = 'cart-items table table-striped table-condensed'>\n  <thead>\n    <tr>\n      <td></td>\n      <td>Sale Quantity</td>\n      <td>Item</td>\n      <td>Price</td>\n      <td>Quantity</td>\n\n    </tr>\n  </thead>\n  <tbody>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.cartItems : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "\n  </tbody>\n</table>\n";
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
  ...
  return "<section class = 'main-view items-view' >\n\n  <h3>"
    + alias2(alias1(((stack1 = ((stack1 = ((stack1 = (depth0 != null ? depth0.items : depth0)) != null ? stack1["1"] : stack1)) != null ? stack1.store : stack1)) != null ? stack1.name : stack1), depth0))
    + "</h3>\n  <p> "
    + alias2(alias1(((stack1 = ((stack1 = ((stack1 = (depth0 != null ? depth0.items : depth0)) != null ? stack1["1"] : stack1)) != null ? stack1.store : stack1)) != null ? stack1.location : stack1), depth0))
    + "</p>\n  <p> On Sale From "
    + alias2(alias1(((stack1 = ((stack1 = (depth0 != null ? depth0.items : depth0)) != null ? stack1["1"] : stack1)) != null ? stack1.start_date : stack1), depth0))
    + " to "
    + alias2(alias1(((stack1 = ((stack1 = (depth0 != null ? depth0.items : depth0)) != null ? stack1["1"] : stack1)) != null ? stack1.end_date : stack1), depth0))
    + " </p>\n\n\n  <ul class=\"nav nav-pills nav-justified sale-view\">\n    <li id= 'sale-items' role=\"presentation\" class=\"active\"><a href=\"#\">On Sale Items</a></li>\n    <li id= 'sale-recipes' role=\"presentation\"><a href=\"#\">Generated Recipes</a></li>\n  </ul>\n\n  <input type=\"text\" class=\"ItemSearch\" placeholder=\"Search Items\">\n</section\n";
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  <div class='col-md-3 col-sm-6 col-xs-12 itemcol'>\n      <div class=\"item panel panel-default\">\n        <div class=\"panel-heading\">\n          <p class=\"panel-title truncate\">"
    ...
    + "\" class = 'img' height='200px' >\n        </div>\n        </a>\n          <button data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\"\n          class=\"item-btn btn btn-mini btn-success glyphicon glyphicon-plus\"\n          type=\"button\">\n          </button>\n          <p> Used Sale Ingredients: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.usedIngredientCount : stack1), depth0))
    + "</p>\n      </div>\n    </div>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;
...
}( true ? module.exports : window.GeoJSON = {}));
